...
% simConf = '../../L-models_90deg/SOF@18d66s_japanese_sensors_fort70_frame_eq_2_CL.mat';
...
% load('../../L-models_90deg/SOF@18d66s_japanese_sensors_fort70_frame_eq_2.mat','Input_struct','x_np','y_np','y_type')
...
% targetConf = '../../L-models_90deg/SOF@18d66s_japanese_sensors_fort70_frame_eq_2_CL.mat';
% targetConf = '../../L-models_90deg/SOF@18d66s_japanese_sensors_cntrlFluxPnts_CL.mat';
% targetConf = '../../L-models_90deg/SOF@18d66s_japanese_sensors_8cntrlFluxPnts_CL.mat';
 targetConf = '../../L-models_90deg/SOF@18d66s_japanese_sensors_6cntrlFluxPnts_CL.mat';


% targetConf = '../L-models_90deg/SOF@18d66s_1_CL.mat';

targetConf = load(targetConf);
% targetEquil = load('../../L-models_90deg/SOF@18d66s_japanese_sensors_cntrlFluxPnts.mat');
% targetEquil = load('../../L-models_90deg/SOF@18d66s_japanese_sensors_8cntrlFluxPnts.mat');
targetEquil = load('../../L-models_90deg/SOF@18d66s_japanese_sensors_6cntrlFluxPnts.mat');
% targetEquil = load('../../L-models_90deg/SOF@18d66s_japanese_sensors_fort70_frame_eq_2.mat');
% targetEquil = load('../L-models_90deg/SOF@18d66s_1.mat');
...
% Prepare data for the 'plotShape' function
meshInfo.p = Input_struct.p; 
meshInfo.e = Input_struct.e;

gapInfo.r = Input_struct.r_sens_gap(1:length(gapIdx));
gapInfo.z = Input_struct.z_sens_gap(1:length(gapIdx));
gapInfo.theta = Input_struct.theta_sens_gap_deg(1:length(gapIdx));

gapIndex.gaps = [1:2, 4:12, 15:19]; 
gapIndex.strike = 20:27;

fig6 = figure('Position', [0, 0, 450, 650]);
% fig6 = figure('Position', [1050, 0, 550, 750]);
%     title('Equilibrium shape')
%     xlabel('R'), ylabel('Z')
hold on
scaleFactor = 30;
for i = 1 : 5 : N
    values.Xpoint = RxZx.value(i, :)';
    values.gaps = Gaps.value(i, :)';
    
    % Plot with gaps
    figure(fig6)
    clf
    plotShape(meshInfo, gapInfo, gapIndex,values , [1 0], [0, 6, -4.5, 4.5], .5);
    hold on
    
    for j = 1 : nSeg
        plot([Rtarget(i, j), Rtarget(i, j)], [Ztarget(i, j)-segError(i, j)/2*scaleFactor, Ztarget(i, j)+segError(i, j)/2*scaleFactor], '-x', 'Color', [1 0 0], 'LineWidth', 2)
    end
    
    plot(Rtarget(i, :), Ztarget(i, :), 'x', 'Color', [0, 1, 0], 'LineWidth', 2)
   
    plot([4, 4], [3.7-0.005*scaleFactor, 3.7+0.005*scaleFactor], '-x', 'Color', [1 0 0], 'LineWidth', 2);
    text(4.1, 3.7, '0.01 Wb/rad');
        
    plot(RXtarget(:, i), ZXtarget(:, i), 'x', 'Color', [0 1 1], 'LineWidth', 2);
    text(4, 4.2, ['time = ' num2str(t(i)) 's'])
...
%% shape snapshots
for i = [round(N/2), N]
figure('Position', [500, 0, 450, 650])
hold on
    values.Xpoint = RxZx.value(i, :)';
    values.gaps = Gaps.value(i, :)';  
    % Plot with gaps
    plotShape(meshInfo, gapInfo, gapIndex, values , [1 0], [1.5, 6, -3.5, 3.5], .5);
    hold on    
    for j = 1 : nSeg
        plot([Rtarget(i, j), Rtarget(i, j)], [Ztarget(i, j)-segError(i, j)/2*scaleFactor, Ztarget(i, j)+segError(i, j)/2*scaleFactor], '-x', 'Color', [1 0 0], 'LineWidth', 2)
    end    
    plot(Rtarget(i, :), Ztarget(i, :), 'x', 'Color', [0, 1, 0], 'LineWidth', 2)  
    plot([5.1, 5.1], [0-0.005*scaleFactor, 0+0.005*scaleFactor], '-x', 'Color', [1 0 0], 'LineWidth', 2);
    text(5.2, 0, '0.01 Wb/rad');       
    plot(RXtarget(:, i), ZXtarget(:, i), 'x', 'Color', [0 1 1], 'LineWidth', 2);
    text(5.1, 0.5, ['time = ' num2str(t(i)) 's'])   
end

% gaps vs flux map
figure('Position', [1000, 0, 450, 650])
plotShape(meshInfo, gapInfo, gapIndex,values , [0 0], [0, 6, -4.5, 4.5], .5);
hold on
pdecont(targetEquil.Input_struct.p,targetEquil.Input_struct.t,targetEquil.x_np(1:length(targetEquil.Input_struct.p)),targetEquil.y_np(strcmp(targetEquil.y_type,'psb_c'))*[1 1]);
...
hold on
fluxSensors = Input_struct.names_sensors(strncmp(Input_struct.names_sensors, 'Flux', 4));
equilFluxes = LinearModel.YEquil(signalIndexByName(fluxSensors, LinearModel.OutputsInfo.Name, LinearModel.OutputsInfo.OutputPosition));
n = length(equilFluxes);

pdemesh(Input_struct.p, Input_struct.e, []), hold on, axis equal

for i = 1 : 10 : n
    plot(Input_struct.r_sens([i, i+9]), Input_struct.z_sens([i, i+9]), '-o') % last 15 sensors are not used in the model
    text(Input_struct.r_sens(i), Input_struct.z_sens(i), Input_struct.names_sensors{i})
end
xlabel('R')
ylabel('Z')
...
r = [Input_struct.r_sens_gap Input_struct.r_sens_gap+.5*cosd(Input_struct.theta_sens_gap_deg)];
z = [Input_struct.z_sens_gap Input_struct.z_sens_gap+.5*sind(Input_struct.theta_sens_gap_deg)];
for i = 1 : length(Input_struct.r_sens_gap)
    plot(r(i,:), z(i,:), '-o') % last 15 sensors are not used in the model
    text(Input_struct.r_sens_gap(i), Input_struct.z_sens_gap(i), int2str(i))
end
xlabel('R')
ylabel('Z')
